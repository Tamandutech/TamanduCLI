...
class NordicUARTService:
    ...
    def _handle_rx_data(self, characteristic: BleakGATTCharacteristic, data: bytearray) -> None:
        try:
            ...
            self._log(f"📨 Received: {message}", "GREEN")
            if self.message_handler:
                self.message_handler(message)
        except UnicodeDecodeError:
            hx = data.hex()
            self._log(f"📨 Received (raw): {hx}", "GREEN")
            if self.message_handler:
                self.message_handler(hx)

    def _handle_disconnect(self, client: BleakClient) -> None:
        if self._on_disconnect_msg:
            self._on_disconnect_msg("🔌 Device disconnected", "RED")
        self.is_connected = False
    ...
    async def connect(self, device: BLEDevice) -> bool:
        try:
            ...
            if not self.rx_characteristic or not self.tx_characteristic:
                self._log("❌ Required characteristics not found", "RED")
                await self.client.disconnect()
                return False
            await self.client.start_notify(self.tx_characteristic, self._handle_rx_data)
            self.is_connected = True
            self._log("✅ Connected to Nordic UART Service", "GREEN")
            return True
        except Exception as e:
            self._log(f"❌ Connection error: {e!s}", "RED")
            return False

    async def send_message(self, message: str) -> bool:
        if not self.is_connected or not self.rx_characteristic:
            self._log("❌ Not connected to device", "RED")
            return False
        try:
            data = message.encode("utf-8")
            await self.client.write_gatt_char(self.rx_characteristic, data, response=True)
            self._log(f"📤 Sent: {message}", "BLUE")
            return True
        except Exception as e:
            self._log(f"❌ Send error: {e!s}", "RED")
            return False
    ...
    async def disconnect(self) -> None:
        if self.client and self.is_connected:
            await self.client.disconnect()
            self.is_connected = False
            self._log("🔌 Disconnected from device", "YELLOW")
